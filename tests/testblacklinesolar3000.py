...
class TestBlacklinesolar(unittest.TestCase):
    ...
    def test_mb_readRegister(self):
        ...
        startRegister = "3C"
        numRegisters = "01"
        self.assertEqual(self.bls.mb_readRegister(subordinateAddress, "\x03", startRegister, numRegisters), "\xFF\x03\x00\x3C\x00\x01\x51\xD8")

        startRegister = "00"
        numRegisters = "03"
        self.assertEqual(self.bls.mb_readRegister(subordinateAddress, "\x04", startRegister, numRegisters), "\xFF\x04\x00\x00\x00\x03\xA5\xD5")

    def test_mb_readHoldingRegisters(self):
        subordinateAddress = "FF"
        startRegister = "3C"
        numRegisters = "01"
        self.assertEqual(self.bls.mb_readHoldingRegisters(subordinateAddress, startRegister, numRegisters), "\xFF\x03\x00\x3C\x00\x01\x51\xD8")

    def test_mb_readInputRegisters(self):
        subordinateAddress = "FF"
        startRegister = "00"
        numRegisters = "03"
        self.assertEqual(self.bls.mb_readInputRegisters(subordinateAddress, startRegister, numRegisters), "\xFF\x04\x00\x00\x00\x03\xA5\xD5")
    ...
    def test_serialNumberQuery(self):
        ...
        self.assertEqual(self.bls.serialNumberCommand(subordinateAddress), "\x02\x04\x00\x00\x00\x03\xB0\x38")

    def test_modelSWCommand(self):
        subordinateAddress = "02"
        self.assertEqual(self.bls.modelSWCommand(subordinateAddress), "\x02\x04\x00\x2B\x00\x02\x01\xF0")
